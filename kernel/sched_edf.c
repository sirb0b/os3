...
//no longer runnable, removed from the linked list, or the edf_rq
...
//checks whether the current task must be pre-emptyed, pre-empted if deadline later than sth in the run-queue
...
//scheduler core call this when preempt done
...
//add the task pointed by p to the linked list of the struct edf_rq;
//called inside sched_setscheduler
